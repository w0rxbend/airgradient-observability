...
import type { CurrentMetric } from "../api/backendClient";
import { statusColor, statusLabel } from "../domain/airQuality";
...
export function GaugeMetric(props: Props) {
  ...
  const color = createMemo(() => statusColor(props.metric.status));
  ...
  return (
    <article
      ...
      aria-label={`${props.metric.label}: ${fmtVal(props.metric.value)} ${props.metric.unit}, ${statusLabel(props.metric.status)}`}
    >
      <header class="gauge-header">
        <span class="gauge-label">{props.metric.label}</span>
        <StatusPill status={props.metric.status} />
      </header>
