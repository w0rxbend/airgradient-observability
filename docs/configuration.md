...
| `infra/oci/docker-compose.vm.build.yml` | OCI local dev | build override for backend and frontend Dockerfiles |
| `infra/oci/Caddyfile` | OCI | HTTPS, Basic Auth, route mapping |
| `infra/oci/frontend.config.json` | OCI frontend | runtime frontend proxy config |
...
| `BACKEND_IMAGE` | yes | `ghcr.io/YOUR_ORG/airgradient-backend:latest` | registry image used by the production backend service |
| `FRONTEND_IMAGE` | yes | `ghcr.io/YOUR_ORG/airgradient-frontend:latest` | registry image used by the production frontend service |
| `BACKEND_URL` | no | `http://backend:8080` | frontend server proxy target; overrides `frontend.config.json` when set |
| `FRONTEND_CONFIG_FILE` | no | `/app/config.json` | JSON config file read by the frontend server proxy |
...
For local OCI-stack development, layer the build override on top of the production compose file:

```bash
docker compose -f docker-compose.vm.yml -f docker-compose.vm.build.yml up -d --build
```

The override builds `backend` from `app/backend/Dockerfile` and `frontend` from `app/frontend/Dockerfile`. The production compose file intentionally uses registry images only.

The frontend server proxies browser calls from `/api/...` to the backend. It resolves the backend URL in this order:

1. `BACKEND_URL`
2. `FRONTEND_CONFIG_FILE` JSON with `backendUrl` or `backend_url`
3. `http://localhost:8081` for isolated mock-server development

OCI mounts `infra/oci/frontend.config.json` to `/app/config.json` by default:
