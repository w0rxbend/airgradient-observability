...
export function TimeSeriesChart(props: Props) {
  ...
  // ── Histogram domain ───────────────────────────────────────────────
  const histBins = createMemo(() => {
    if (props.points.length === 0) return [];
    const vals = props.points.map(([, v]) => v);
    const lo = Math.min(...vals);
    const hi = Math.max(...vals);
    const bw = (hi - lo || 1) / N_BINS;
    ...
    for (const v of vals) {
      const i = Math.min(N_BINS - 1, Math.floor((v - lo) / bw));
      bins[i].count++;
    }
    return bins;
  });
      ...
      >
        ...
        >
          ...
          <Show when={chartType() !== "histogram" && cursor() && hoverPt()}>
            ...
            >
              <div class="chart-tooltip-value">{formatValue(hoverPt()!.val)} {props.unit}</div>
              <div class="chart-tooltip-time">{formatTimeFull(hoverPt()!.ts)}</div>
            </div>
          </Show>
